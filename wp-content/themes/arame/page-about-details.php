<?php
/*
Template Name: About Details
*/
get_header();
?>

<main>
  <section id="service-hero">
    <div class="container-xl">
      <h1 class="hero-title">
        High-Tailored IT Solutions and Dedicated Software Services
      </h1>
      <p class="hero-subtitle">
        View Technologies provides teams of dedicated software engineers who
        know how to satisfy client testing requirements with efficient
        management skills and software product engineering methodologies.
      </p>
    </div>
  </section>

  <!-- HERO -->
  <section id="about-hero">
    <div class="blob"></div>

    <div class="container">
      <div class="row align-items-center">
        <div class="col-lg-6">
          <h1>We Build Future-Ready IT Solutions</h1>
          <p>
            OptiTech delivers cutting-edge technology services that help
            businesses grow, innovate, and stay ahead in an ever-evolving
            digital landscape.
          </p>
        </div>
        <div class="col-lg-6 about-img">
          <img src="https://images.unsplash.com/photo-1522071820081-009f0129c71c" alt="Team Photo" />
        </div>
      </div>
    </div>
  </section>

  <!-- WHO WE ARE -->
  <section class="py-5">
    <div class="container">
      <div class="row align-items-center">
        <div class="col-lg-6 mb-4">
          <img src="https://images.unsplash.com/photo-1551434678-e076c223a692" class="img-fluid rounded" alt="" />
        </div>

        <div class="col-lg-6">
          <h2 class="section-title mb-3">Who We Are</h2>
          <p class="mb-4">
            We are a team of passionate developers, designers, engineers, and
            strategists committed to building technology that makes an impact.
          </p>

          <ul class="info-list list-unstyled">
            <li><i class="fa-solid fa-check"></i> 1+ Years of Experience</li>
            <li>
              <i class="fa-solid fa-check"></i> 1+ Successful Projects
              Delivered
            </li>
          </ul>
        </div>
      </div>
    </div>
  </section>

  <!-- STATS -->
  <section class="py-5 bg-light">
    <div class="container">
      <div class="row text-center">
        <div class="col-lg-3 col-6 mb-4">
          <div class="stats-box">
            <h2>1+</h2>
            <p>Projects Delivered</p>
          </div>
        </div>

        <div class="col-lg-3 col-6 mb-4">
          <div class="stats-box">
            <h2>1+</h2>
            <p>Active Clients</p>
          </div>
        </div>

        <div class="col-lg-3 col-6 mb-4">
          <div class="stats-box">
            <h2>10+</h2>
            <p>Team Members</p>
          </div>
        </div>

        <div class="col-lg-3 col-6 mb-4">
          <div class="stats-box">
            <h2>1+</h2>
            <p>Years of Experience</p>
          </div>
        </div>
      </div>
    </div>
  </section>

  <!-- WHY CHOOSE US -->
  <section class="py-5">
    <div class="container">
      <div class="text-center mb-5">
        <h2 class="section-title">Why Choose Us?</h2>
        <p class="text-muted">
          We bring innovation, reliability, and expertise to every project.
        </p>
      </div>

      <div class="row g-4">
        <div class="col-lg-4">
          <div class="why-card">
            <i class="fa-solid fa-lightbulb"></i>
            <h4>Innovative Solutions</h4>
            <p>We create smart digital products that solve real problems.</p>
          </div>
        </div>

        <div class="col-lg-4">
          <div class="why-card">
            <i class="fa-solid fa-shield-halved"></i>
            <h4>Trusted & Secure</h4>
            <p>We follow industry-best practices to ensure security.</p>
          </div>
        </div>

        <div class="col-lg-4">
          <div class="why-card">
            <i class="fa-solid fa-people-group"></i>
            <h4>Expert Team</h4>
            <p>Skilled professionals dedicated to your success.</p>
          </div>
        </div>
      </div>
    </div>
  </section>

  <!-- Team -->
  <div class="text-center mb-5">
    <h2 class="section-title">Meet Our Team</h2>
  </div>

  <div class="team-grid-container">
    <div class="member-card">
      <div class="member-photo-wrapper">
        <img
          src="https://media.istockphoto.com/id/1626847852/photo/a-happy-beautiful-blonde-businesswoman-working-on-her-computer.jpg?s=612x612&w=0&k=20&c=btZhAJgSUBq6zaemDaNAQTRVYOQbEmYZ3g-J4y3IiuQ="
          alt="Mary Brown" />
      </div>
      <p class="member-role">creative leader</p>
      <h3 class="member-name">Mary Brown</h3>
      <p class="member-description">
        Glavi amet ritnisl libero molestie ante ut fringilla purus eros quis
        glavrid from dolor amet iquam bibendum
      </p>
      <div class="member-social-links">
        <a href="#"><i class="fab fa-facebook-f"></i></a>
        <a href="#"><i class="fab fa-twitter"></i></a>
        <a href="#"><i class="fab fa-instagram"></i></a>
        <a href="#"><i class="fab fa-linkedin-in"></i></a>
      </div>
    </div>
    <div class="member-card">
      <div class="member-photo-wrapper">
        <img src="https://images.unsplash.com/photo-1607746882042-944635dfe10e" alt="Mary Brown" />
      </div>
      <p class="member-role">creative leader</p>
      <h3 class="member-name">Mary Brown</h3>
      <p class="member-description">
        Glavi amet ritnisl libero molestie ante ut fringilla purus eros quis
        glavrid from dolor amet iquam bibendum
      </p>
      <div class="member-social-links">
        <a href="#"><i class="fab fa-facebook-f"></i></a>
        <a href="#"><i class="fab fa-twitter"></i></a>
        <a href="#"><i class="fab fa-instagram"></i></a>
        <a href="#"><i class="fab fa-linkedin-in"></i></a>
      </div>
    </div>
    <div class="member-card">
      <div class="member-photo-wrapper">
        <img src="https://images.unsplash.com/photo-1544005313-94ddf0286df2" alt="Mary Brown" />
      </div>
      <p class="member-role">creative leader</p>
      <h3 class="member-name">Mary Brown</h3>
      <p class="member-description">
        Glavi amet ritnisl libero molestie ante ut fringilla purus eros quis
        glavrid from dolor amet iquam bibendum
      </p>
      <div class="member-social-links">
        <a href="#"><i class="fab fa-facebook-f"></i></a>
        <a href="#"><i class="fab fa-twitter"></i></a>
        <a href="#"><i class="fab fa-instagram"></i></a>
        <a href="#"><i class="fab fa-linkedin-in"></i></a>
      </div>
    </div>
  </div>

  <!-- TEAM -->
  <section class="py-5 bg-light">
    <div class="container">
      <div class="text-center mb-5">
        <h2 class="section-title">Meet Our Team</h2>
      </div>

      <div class="row g-4">
        <div class="col-lg-3 col-md-6">
          <div class="team-card">
            <img src="https://images.unsplash.com/photo-1607746882042-944635dfe10e" alt="" />
            <h5>Sarah John</h5>
            <p>CEO & Founder</p>
          </div>
        </div>

        <div class="col-lg-3 col-md-6">
          <div class="team-card">
            <img src="https://images.unsplash.com/photo-1544005313-94ddf0286df2" alt="" />
            <h5>Michael Lee</h5>
            <p>CTO</p>
          </div>
        </div>

        <div class="col-lg-3 col-md-6">
          <div class="team-card">
            <img src="https://images.unsplash.com/photo-1494790108377-be9c29b29330" alt="" />
            <h5>Emily Carter</h5>
            <p>Project Manager</p>
          </div>
        </div>

        <div class="col-lg-3 col-md-6">
          <div class="team-card">
            <img src="https://images.unsplash.com/photo-1529626455594-4ff0802cfb7e" alt="" />
            <h5>Benjamin Wilson</h5>
            <p>Lead Developer</p>
          </div>
        </div>
      </div>
    </div>
  </section>
</main>

<?php get_footer(); ?>